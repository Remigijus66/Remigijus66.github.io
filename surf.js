...
const emitDots = () => {
  for (let j = 40; j <= 300; j += 50) {
    for (let i = 40; i <= 300; i += 50) {
      ...
      dots[variableName] = new DotDrawer('container', id, 'gray', 2, Math.floor(Math.random() * 5) + 1, i * 1 + Math.floor(Math.random() * 50) + 50, j * 1 + Math.floor(Math.random() * 50) - 50, direction, 50 + Math.floor(Math.random() * 200));
